...
for c,row in enumerate(csv.DictReader(open(name))):
    if row['id'] not in fea:
        fea[row['id']]=[]
        feax[row['id']]=set()
    if row['resource_type'].split()[-1] not in feax[row['id']]:
        feax[row['id']].add(row['resource_type'].split()[-1])     
        value='%s:1'%(row['resource_type'].split()[-1])
