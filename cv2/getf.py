...
s=pd.DataFrame(X,columns=['fea%d'%i for i in range(X.shape[1])])
s.to_csv('train1x.csv',index=False)

s=pd.DataFrame(Xt,columns=['fea%d'%i for i in range(Xt.shape[1])])
s.to_csv('test1x.csv',index=False)
