...
def kfold_cv(X_train, y_train,k):
    ...
    for train_index, test_index in kf:
        ...
        clf=xgb_classifier(eta=0.1,gamma=1e-3,col=0.2,min_child_weight=0.5,depth=7,num_round=160)
...
X=np.array(xx).T
print X.shape, y.shape
train=pd.read_csv('../explore/train2.csv')
X=np.hstack([X,train.as_matrix()])
#X=train.as_matrix()
