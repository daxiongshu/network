...
def kfold_cv(X_train, y_train,k):
    ...
    for train_index, test_index in kf:
        ...
        clf=RandomForestClassifier(n_jobs=-1,max_depth=21,max_features=200,n_estimators=100)
...
#X=np.hstack([X,pd.read_csv('cvbackup/va_0.514812.csv').drop(['id','real'],axis=1).as_matrix()])
