...
# A, paths
train=['../input/train.csv']#,'../encode/encode4/dp_encode.csv']
test=train
trainsvm=['../sparse/rebuild1.svm','../sparse/rebuild2.svm','../sparse/rebuild3.svm','../sparse/rebuild4.svm']
testsvm=trainsvm
submission = 'ftrl1x.csv'  # path of to be outputted submission file
    ...
    for t,  x, y in data(train,trainsvm, D,cc=1):  # data is a generator
        ...
        if count%1000==0:
            ...
            print('%s\tencountered: %d\tcurrent logloss: %f y: %f' % (
                datetime.now(), count, loss/(count-2894),y))
...
with open(submission, 'w') as outfile:
    outfile.write('id,fault_severity,real\n')
